riot.tag('menu', '<div class="swiper-container"> <div class="swiper-wrapper"> <div class="swiper-slide" each="{arr, i in mappedKeys}"> <div class="vendor-wrapper" each="{item, j in arr}"> <div class="vendor-name">{item}</div> <div class="product-wrapper" each="{prd, k in window.menuInfo.form_json[item]}"> <div class="content-wrapper"> <div class="img-wrapper" riot-style="background-image: url(\'{prd.link}\')"> <div class="{ prd.qty? \'img-qty-cover visible\':\'img-qty-cover\'}">{prd.qty}</div> </div> <div class="product-desc"> <h3 class="product-name">{prd.name}</h3> <span class="product-price">Rs. {prd.price}</span> </div> <div class="submit-btn" onclick="{ add }">ADD</div> <div class="{ prd.qty? \'btn-group show\':\'btn-group\'}"> <input type="button" class="counter minus" value="-" onclick="{ minus }"></input> <input type="button" class="counter plus" value="+" onclick="{ add }"></input> </div> </div> </div> </div> </div> </div> <div class="swiper-pagination"></div> <div class="{isSubmitted? \'cart-container visible\':\'cart-container\'}" id="cart"> <i class="fa fa-shopping-cart shopping-card-icon"></i> <img src="./images/cart.png"> <div class="cart-count" id="count">{window.count}</div> <span class="submit" riot-style="font-size: {isSubmitted?\'16px\':\'20px\'}">{isSubmitted? \'Form already submitted.\':\'SUBMIT\'}</span> </div> </div>', function(opts) {
